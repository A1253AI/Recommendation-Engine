...
if st.button("Get Recommendations", type="primary"):
    if user_input:
        with st.spinner("Finding perfect matches..."):
            ...
            if method == "Hybrid":
                results = recommend_anime(user_input, top_n)
            elif method == "Semantic":
                results = recommend_anime(user_input, top_n, faiss_weight=1, tfidf_weight=0)
            else:
                results = recommend_anime(user_input, top_n, faiss_weight=0, tfidf_weight=1)
            
            # Display results
            if not results.empty:
                st.success(f"Found {len(results)} matching anime")
                for _, row in results.iterrows():
                    with st.expander(f"{row[ANIME_NAME_COL]}  {row.iloc[RATING_COL_IDX]:.1f}"):
                        cols = st.columns([1, 3])
                        with cols[0]:
                            try:
                                st.image(row[IMAGE_COL], width=150, 
                                       caption=row[ANIME_NAME_COL])
                            except:
                                st.image("https://via.placeholder.com/200x300?text=Image+Not+Found",
                                       width=200)
                        with cols[1]:
                            st.markdown(f"**Episodes:** {row.iloc[EPISODES_COL_IDX]}")
                            genres = ', '.join([col for col in df.columns[GENRE_START_COL_IDX:] 
                                              if row[col] == 1])
